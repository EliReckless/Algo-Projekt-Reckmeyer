...
# 9269 bleiben übrig, was auf jeden Fall ausreichend sein sollte.

prop.table(table(okcupid_profiles$sex))
# Klassenverteilung ist relativ ähnlich und sollte so in Ordnung sein.
```
...
temp <- sample(nrow(okcupid_profiles), nrow(okcupid_profiles)*.7)
...
# Verteilung der Geschlechter ist im Trainings- und Testdatensatz auch ok.
...
```
# Hier habe ich zunächst mal alle der Variablen verwendet, die ich mir im letzten Schritt rausgeholt hat und dann nach und nach die rausgegeben, die das Model nicht verbessert haben. 
...
accuracy_Test
```
## Das Model auf das ich mich am Eende festgelegt habe hat eine Accuracy von 86% was kein schlechter Wert ist. Ich denke man könnte noch einen höheren Wert erreichen, wenn man die Essay-Fragen auf eine sinnvolle Weise inkludiert.
...
rec
```
## Diese Werte sind meiner Meinung nach auch völlig akzeptabel, sowohl precision als auch recall liegen bei 87%.
